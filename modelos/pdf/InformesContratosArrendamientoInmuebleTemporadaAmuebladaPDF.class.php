<?php
/*
InformesContratosArrendamientoInmuebleTemporadaAmuebladaPDF.class.php, v 2.4 2013/05/13

InformesContratosArrendamientoInmuebleTemporadaAmuebladaPDF - Clase para crear informes de los contratos de arrendamiento de los inmuebles en formato PDF
...
/**
*
* InformesContratosArrendamientoInmuebleTemporadaAmuebladaPDF
*
* Clase para crear informes de los contratos de arrendamiento de los inmuebles en formato PDF
*
* @author   Angel Luis Berasuain Ruiz <klaimir@hotmail.com>
* @version  InformesContratosArrendamientoInmuebleTemporadaAmuebladaPDF.class.php, v 2.4 2013/05/13
* @access   public
*/

class InformesContratosArrendamientoInmuebleTemporadaAmuebladaPDF extends InformesContratosArrendamientoInmueblePDF
{	
	/**
	 * Constructor
	 *
	...
	function CuerpoFicha($datos) 
	{
		...
		$this->pdf->MultiCell(190,5,"CONTRATO DE ARRENDAMIENTO POR  TEMPORADA DE VIVIENDA AMUEBLADA",0,'C',0);
		$this->pdf->Ln(5);		
		...
		$this->pdf->Ln(5);
		$this->pdf->SetFont("arial", "", 8);
		$this->pdf->MultiCell(195,4,"Se hace constar por el arrendatario que dicho arrendamiento es temporal. Por ello, no es su intención fijar su domicilio habitual en la vivienda objeto de este contrato.",0,'J',0);
		$this->pdf->Ln(5);	
		...
		$this->pdf->Ln(5);
		$this->pdf->SetFont("arial", "", 8);
		$this->pdf->MultiCell(195,4,"Las partes contratantes, conforme a lo dispuesto en el Art.9 del Real Decreto Ley 29/1994 de 24 de Noviembre, pactan libremente que este contrato tendrá una duración exclusiva de once meses, por lo que comenzará el ".$datos['dia_fecha_inicio_contrato']." del mes de ".$datos['mes_fecha_inicio_contrato']." y finalizando el ".$datos['dia_fecha_fin_contrato']." del mes de ".$datos['mes_fecha_fin_contrato']." del año ".$datos['anio_fecha_fin_contrato'],0,'J',0);
		$this->pdf->Ln(5);
		$this->pdf->SetFont('Arial','B',13);
		...
		$this->pdf->Ln(5);
		$this->pdf->SetFont('Arial','B',13);
		$this->pdf->MultiCell(190,5,"7.- PROHIBICIONES.",0,'L',0);
		$this->pdf->Ln(5);
		$this->pdf->SetFont("arial", "", 8);
		...
		$this->pdf->Ln(5);
		$this->pdf->SetFont('Arial','B',13);
		$this->pdf->MultiCell(190,5,"8.- CONSERVACIÓN DE LA VIVIENDA.",0,'L',0);
		$this->pdf->Ln(5);
		$this->pdf->SetFont("arial", "", 8);
		...
		$this->pdf->Ln(5);
		$this->pdf->MultiCell(195,4,"Las reparaciones que sean necesarias por el uso ordinario y que tengan la consideración de “pequeñas reparaciones”, serán de la exclusiva cuenta del arrendatario. A tal fin se pacta que las que no excedan de media mensualidad de renta, serán calificadas como “pequeñas reparaciones”.",0,'J',0);
		$this->pdf->Ln(5);
		$this->pdf->MultiCell(195,4,"Igualmente las reparaciones que sean consecuencia de un daño doloso o de mal uso de la casa arrendada, serán de la exclusiva cuenta del arrendatario.",0,'J',0);
		$this->pdf->Ln(5);
		...
		$this->pdf->Ln(5);
		$this->pdf->SetFont('Arial','B',13);
		$this->pdf->MultiCell(190,5,"9.- SERVICIOS Y SUMINISTROS.",0,'L',0);
		$this->pdf->Ln(5);
		$this->pdf->SetFont("arial", "", 8);
		$this->pdf->MultiCell(195,4,"A los efectos previstos en el artículo 20.2 de la Ley de Arrendamientos Urbanos el arrendatario se obliga al abono del consumo de agua, luz y gas. ",0,'J',0);
		$this->pdf->Ln(5);
		$this->pdf->MultiCell(195,4,"Se establece expresamente que las cantidades a abonar por los servicios y suministros mencionados anteriormente, tendrán el carácter de cantidades asimiladas a la renta.",0,'J',0);
		$this->pdf->Ln(5);
		$this->pdf->SetFont('Arial','B',13);
		$this->pdf->MultiCell(190,5,"10.- LEGISLACIÓN  APLICABLE.",0,'L',0);
		$this->pdf->Ln(5);
		$this->pdf->SetFont("arial", "", 8);
		...
		$this->pdf->Ln(15);
		$this->pdf->SetFont('Arial','B',13);
		$this->pdf->MultiCell(190,5,"11.- JURISDICCIÓN. ",0,'L',0);
		$this->pdf->Ln(5);
		$this->pdf->SetFont("arial", "", 8);
