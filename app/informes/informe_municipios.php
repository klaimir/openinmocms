...
<?php
	...
	if($_POST)
	{
		...
		if($_POST['provincia'] == "")
		{
			...
			$_SESSION['s_busq_informes_municipios_todos'] = $sql_ninguno;
		}
		else
		{		
			...
			$sql_todos .= $sql_region. $sql_provincia;
			// Establecemos las búsquedas personalizadas
			$_SESSION['s_busq_informes_municipios_todos'] = $sql_todos;
		}			
	...
	$pages = new Paginator('municipios');  
	...
	$_SESSION['display_pages_busq_informes_municipios']=$pages->display_pages();
	$_SESSION['display_menu_pages_busq_informes_municipios']='<span style="margin-left:25px"> '. $pages->display_jump_menu()  
. $pages->display_items_per_page() . '</span>';

	// Paginación en el sql
	$_SESSION['s_busq_informes_municipios_todos'].=$sql_pagination_limit;
?>
<?php Interfaz::PlantillaGenerica("informes","informe_municipios"); ?>
<?php
