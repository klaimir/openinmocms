...
<?php
	...
	if($_POST)
	{
		...
		if($_POST['poblacion'] == "")
		{
			$hayerrores = true; $errores[$i++] = "Debe seleccionar un municipio para la búsqueda";
			$_SESSION['s_busq_informes_zonas_todos'] = $sql_ninguno;
		}
		else
		{		
			...
			$sql_todos .= $sql_region. $sql_provincia. $sql_poblacion;
			// Establecemos las búsquedas personalizadas
			$_SESSION['s_busq_informes_zonas_todos'] = $sql_todos;
		}			
	...
	$_SESSION['display_pages_busq_informes_zonas']=$pages->display_pages();
	$_SESSION['display_menu_pages_busq_informes_zonas']='<span style="margin-left:25px"> '. $pages->display_jump_menu()  
. $pages->display_items_per_page() . '</span>';

	// Paginación en el sql
	$_SESSION['s_busq_informes_zonas_todos'].=$sql_pagination_limit;
?>
<?php Interfaz::PlantillaGenerica("informes","informe_zonas"); ?>
<?php
