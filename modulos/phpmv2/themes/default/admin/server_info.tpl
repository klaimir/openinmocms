{assign var=ok value="<img src='$PMV_THEME/images/ok.gif' />"}
{assign var=error value="<img src='$PMV_THEME/images/no.gif' />"}
{assign var=warning value="<img src='$PMV_THEME/images/warning.gif' />"}
...
<table class="infos">
	<tr>
		...
		<td>{if $server.php_ok}{$ok}{else}{$error}{/if}</td>
	</tr>
	<tr>
		<td class="libelle">Mysql extension</td>
		<td>{if $server.mysql_ok}{$ok}{else}{$error}{/if}</td>
	</tr>  
	<tr>
		<td class="libelle">PHP-XML extension (utf8_decode function)</td>
	    <td>{if $server.php_xml}{$ok}{else}{$error}{/if}</td>
	</tr>
	<tr>
		<td class="libelle">
...
<table class="infos">
	<tr>
	    ...
	    <td>{if $server.gd_ok}{$ok}{else}{$warning}{/if}</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			...
			{if $server.memory_ok}{$ok}{else}{$warning}{/if}	
			{$server.memory} 
		</td>
	</tr>
	<tr>
		<td class="libelle">set_time_limit() {'admin_allowed'|translate}</td>
		<td>{if $server.set_time_limit_ok}{$ok}{else}{$warning}{/if}</td>
	</tr>
	<tr>
		<td class="libelle">mail() {'admin_allowed'|translate}</td>
		<td>{if $server.mail_ok}{$ok}{else}{$warning}{/if}</td>
	</tr>
</table>
