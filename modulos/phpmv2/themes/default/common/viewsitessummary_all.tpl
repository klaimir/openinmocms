...
<tr onmouseover="pointer(this)" onclick="loadUrl('{$url_mod_site}&amp;site={$info.id}&amp;mod=view_visits')">
{cycle values="," assign=style}
	<td class="{$style}align"  width="130px">{$info.site_name}</td>
	<td class="{$style}"><strong>{$info.nb_vis}</strong></td>
...
<tr>{cycle values="damierlight" assign=style}
	<td class="{$style}"><strong>T<small>OTAL</small></strong></td>
