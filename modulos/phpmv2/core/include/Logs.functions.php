...
// $Id: Logs.functions.php 218 2007-06-02 07:15:48Z cmil $

function isPrefixTag ($pageName) {
	$tabTag = split(",", PREFIX_ALL_TAG);
	$ret = false;
	foreach ($tabTag as $info) {
		if (substr($pageName, 0, strlen($info)) === $info) {
			$ret = true;
			break;
		}
	}
...
function getPrefixTag ($pageName) {
	$tabTag = split(",", PREFIX_ALL_TAG);
	$ret = "";
	foreach ($tabTag as $info) {
		if (substr($pageName, 0, strlen($info)) === $info) {
...
function deletePrefixTag ($pageName) {
	$tabTag = split(",", PREFIX_ALL_TAG);
	$ret = $pageName;
	foreach ($tabTag as $info) {
		if (substr($pageName, 0, strlen($info)) === $info) {
			$ret = substr($pageName, strlen($info));
			break;
		}
	}
	return $ret;
...
function setCookieFromPear ($aPearCookie) {
	$v_secure = false;
	if (($aPearCookie['secure'] != null) && ($aPearCookie['secure'] == "true")) {
		$v_secure = true;
	}
	setcookie ($aPearCookie['name'], $aPearCookie['value'], strtotime($aPearCookie['expires']),
			$aPearCookie['path'], $aPearCookie['domain'], $v_secure);
}
				...
				if (defined(PMV_REWRITE_MODE) && (PMV_REWRITE_MODE == 'PEAR_REQUEST'))
				{
					...
					$a->sendRequest();
	 				...
	 				foreach ($a->getResponseHeader() as $key => $value) {
	 					if ($key != "transfer-encoding") { // IE does not support this parameter
							header ($key.": ".$value);
	 					}
					}
...
function processParams($url, $siteInfo)
{
	...
	// always include these Id
	$a_toInclude = array();
	$a_toInclude[] = PARAM_URL_PARTNER;
	$a_toInclude[] = PARAM_URL_NEWSLETTER;
	...
	// if there is a query we keep only the accepted parameters in that query
		...
		foreach($queryEx as $value)
		{
			...
			if( sizeof($varAndValue) == 2
				...
				(
					...
					||
					// keep all except some parameters
					($choice == 'except' && !in_array($varAndValue[0], $a_varsToExclude))
					||
					// keep all parameters, except those that are always excluded (phpsessid, etc.)
					($choice == 'all' && !in_array($varAndValue[0], $a_toExclude))
					||
					// keep always some parameters (partner parameter, newsletter parameter)
					in_array( $varAndValue[0], $a_toInclude)
				)
			)
			{
...
function displayPageWhenEmptyGet()
{
	...
	print("
			...
			<p>".str_replace("Matthieu Aubry", "<a href='http://matthieu.net/'>Matthieu</a> (<a href='http://matthieu.net/blog/'>blog</a>)",$GLOBALS['lang']['contacts_auteur'])."</p>
